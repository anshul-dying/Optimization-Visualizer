...
class SVM:
    ...
    def fit(self, X, y):
        ...
        for iteration in range(self.max_iter):
            ...
            # Hinge loss
            distances = y * self._decision_function_batch(X)
            loss = 0.5 * np.dot(self.alpha, y) # simplified dual loss
    ...
    def _decision_function_batch(self, X):
        return np.sum(self.alpha * self.y * self._kernel(self.X, X), axis=1) + self.b
